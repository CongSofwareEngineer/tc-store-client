    ...
    openModal({
      body: (
        <div className='w-[90%] flex justify-center items-center h-[90%]'>
          <MyImage alt={src} className='!w-auto !h-full !max-w-full !max-h-full' src={detectImg(src || srcHover)} />
        </div>
      ),
    ...
    >
      ...
      {srcHover && !isMobile && (
        <div className='absolute-center transition-all duration-200 flex justify-center items-center w-full h-full bg-black/50 cursor-pointer text-white inset-0'>
          <div className='text-xs  m-auto flex items-center gap-1' onClick={() => handleViewFull('')}>
            <LuEye />
...
const ImgMoreProduct = ({ data, onHover = () => {} }: { data: IImageProduct[]; onHover?: (param?: IImageProduct) => void }) => {
