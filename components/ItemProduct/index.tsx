...
const ItemProduct = ({ item, callback = () => {}, className, showSold = false, showFeedback = false, showDiscount = true, href = '', noClick = false }: ItemType) => {
	...
	const renderContent = () => {
		return (
			<div className={`  group relative item-list cursor-pointer px-3 md:pt-6 pt-4 md:pb-4 pb-3 gap-3 flex items-center justify-between flex-col ${styles['item-coffee']} ${className}`}>
				{showDiscount && item?.disCount > 0 && <div className="absolute text-black right-0 top-4 bg-green-300   px-2  rounded-l-md z-[1]">-{item?.disCount}%</div>}

				<div className="m-auto max-w-[85%] relative w-full aspect-square  overflow-hidden">
					<MyImage src={detectImg(item?.imageMain || images.userDetail.iconUserDetail, 350)} alt={`item-${item?.name || href}`} className="!relative  !h-auto group-hover:scale-110 transform transition duration-300 ease-in-out select-none" />
