  ...
  return (
    <div className='bg-white w-full  flex flex-col   border-[1px] shadow-gray1 border-gray-300 md:p-3 px-4 py-4'>
      <div className='flex w-full gap-2'>
        <div>
          <MyImage alt='icon-optionPayment-bill' className='  !w-[25px] !h-[25px]' src={images.icon.iconOptionPayment} />
        </div>
      ...
      <div className='w-full flex flex-col  md:gap-2 gap-2'>
        {listOptions.map((e, index) => {
          return (
            <div key={`option-payment-${index}`} className='relative flex gap-2 items-center'>
              <div className='cursor-pointer flex gap-2  md:text-[16px] text-[14px] items-center' onClick={() => onChangeOptions(e)}>
                <MyImage alt={`checkbox-${e.name}`} className=' !w-5 !h-5' src={getIcon(e.value)} />
                {e.icon && <MyImage alt={`icon-${e.name}`} className=' !w-[18px] !h-[18px]' src={e.icon} />}
                <span>{e.name}</span>
              </div>
