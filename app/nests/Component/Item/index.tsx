    ...
    >
      ...
      <div className="p-4 hover:shadow-lg cursor-pointer group bg-white rounded-lg w-full flex flex-col items-center justify-center">
        ...
        <div className="w-full gap-1 flex flex-col mt-2 te">
          ...
          <div className=" w-full text-green-400 line-through md:text-sm text-xs   ">
            {`${formatPriceBase(data?.price || 150, data?.disCount)} VNĐ`}
          </div>
          ...
          <span className="text-[11px] md:mt-1 ">{`${translate(
            'productDetail.sold'
          )}  ${numberWithCommas(data?.sold || '0')}`}</span>
        </div>
      </div>
    </Link>
  )
