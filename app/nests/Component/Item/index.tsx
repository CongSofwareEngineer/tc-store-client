...
const ItemNest = ({ data }: { data: Record<string, any> | null }) => {
  ...
  return (
    <Link className='w-full cursor-pointer relative' href={`/shop/${data?.keyName}`}>
      ...
      <div className='p-4 hover:shadow-lg cursor-pointer group bg-white rounded-lg w-full flex flex-col items-center justify-center'>
        ...
        <div className='w-full gap-1 flex flex-col mt-2 te'>
          <p className='w-full md:text-medium font-bold whitespace-nowrap overflow-hidden text-ellipsis'>{data?.name}</p>
          <div className=' w-full text-green-400 line-through md:text-sm text-xs   '>{`${formatPriceBase(data?.price || 150, data?.disCount)} VNĐ`}</div>
          ...
          <span className='text-[11px] md:mt-1 '>{`${translate('productDetail.sold')}  ${numberWithCommas(data?.sold || '0')}`}</span>
        </div>
      </div>
    </Link>
  )
